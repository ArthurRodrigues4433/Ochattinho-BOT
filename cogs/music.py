...
def parse_playlist(info: dict):
    ...
    for entry in info.get("entries", []):
        if not entry:
            continue
        songs.append({"id": entry["id"], "title": entry.get("title", "Sem título")})
    return songs
...
class Music(commands.Cog):
            ...
            async with self.queue_lock:
                with yt_dlp.YoutubeDL(ydl_opts) as ydl:  # type: ignore
                    ...
                    if info.get("_type") == "playlist":
                        songs = parse_playlist(info)  # type: ignore

                        for song in songs:
                            self.music_queue.append(song)

                        await ctx.send(
                            f"📀 Playlist adicionada ({len(songs)} músicas)."
                        )

                    # SINGLE
                    else:
                        self.music_queue.append(
                            {"id": info["id"], "title": info.get("title", "Sem título")}
                        )
    ...
    async def play_next(self, ctx):
        ...
        with yt_dlp.YoutubeDL(ydl_opts) as ydl:  # type: ignore
            info = ydl.extract_info(
                f"https://www.youtube.com/watch?v={song['id']}", download=False
            )
